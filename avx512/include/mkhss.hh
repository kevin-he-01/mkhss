...
#include "rns_fbe.hh"
using namespace rns_fbe;
constexpr bool use_rns_multiplication = true;
#else
#include "fixed_base_exp.hh"
using namespace fixed_base_exp;
constexpr bool use_rns_multiplication = false;
#endif
...
namespace mkhss {
    ...
    struct crs {
        ...
        mutable std::optional<RNSCtx<N2_LIMBS>> rns_ctx; // RNS context for n^(w+1)
    };
    ...
    inline void precompute_share(const crs &crs, public_key &pk, int num_shares) {
        ...
        pk.g_fbe.precompute(crs.g, crs.n_w_1, crs.params.sec_param * 2, window_size, crs.rns_ctx);
        pk.f_fbe.precompute(pk.f, crs.n_w_1, crs.params.sec_param * 2, window_size, crs.rns_ctx);
        pk.precomputed = true;
    }
    ...
    struct input_share {
        ...
        inline void precompute(const crs &crs, int expected_usage_count, int leeway = 0) {
            ...
            c0_fbe.precompute(c0, crs.n_w_1, crs.params.exponent_bits + leeway, get_optimal_window_size(crs.params.exponent_bits, expected_usage_count), crs.rns_ctx);
            c1_fbe.precompute(c1, crs.n_w_1, crs.params.bprime_bits + leeway, get_optimal_window_size(crs.params.bprime_bits, expected_usage_count), crs.rns_ctx);
            precomputed = true;
        }
    ...
    public:
        ...
        inline void precompute(const crs &crs, int explinencs_count) {
            ...
            f_fbe.precompute(f, crs.n_w_1, crs.params.sec_param * 2, window_size, crs.rns_ctx);
            precomputed = true;
        }
