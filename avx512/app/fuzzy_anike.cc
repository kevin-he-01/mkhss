#include "fuzzy_anike.hh"

using namespace fuzzy_anike;
...
int hamming_distance(const std::vector<bool> &x, const std::vector<bool> &y, int l) {
    // b-bit chunkwise Hamming distance (each b-bit chunk is a symbol)
...
void output_benchmark_result(const crs &crs, double keygenA_time, double keygenB_time, double keyderA_time, double keyderB_time) {
    ...
    std::cout << "length: " << crs.params.total_length << std::endl;
...
int main(int argc, char **argv) {
    ...
    int length = argc == 1 ? 32 : atoi(argv[1]);
    int word_length = 4;
    // int threshold = length / 3; // DEBUG
    int threshold = length; // DEBUG
    int bit_length = length * word_length;
    crs crs;
    parameters params;
    public_encoding pe_A, pe_B;
    private_state st_A, st_B;
    create_parameters(params, 128, 128, 3072, length, word_length);
    ...
    for (int i = 0; i < params.total_length; i++) {
        x[i] = rand() % 2;
        y[i] = x[i] ^ (rand() <= (uint64_t) RAND_MAX * threshold / (bit_length));
        // y[i] = x[i] ^ (rand() <= (uint64_t) i * i * RAND_MAX * threshold / (bit_length * params.total_length));
        // y[i] = x[i];
    }
    std::cerr << "x: ";
    for (int i = 0; i < params.total_length; i++) {
        if (i % params.word_length == 0) {
            std::cerr << " ";
        }
        std::cerr << x[i];
    }
