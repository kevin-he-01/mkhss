...
// fmpz_class.cc
extern std::optional<RNSCtx<N2_LIMBS>> *rns_ctx;
...
namespace mkhss {
    ...
    void setup(crs &crs, const parameters &params, mpz_srcptr n) {
        ...
        crs.rns_ctx.emplace(io, crs.n_w_1);
        crs.n_w_1.set_is_modulus(true);
        rns_ctx = &crs.rns_ctx;

        // Set up modulus context
    ...
    void rms_mult(const crs &crs, PRF &prf, uint64_t unique_id, const input_share &b, const memory_share &a, memory_share &result, std::optional<int> max_log_y) {
        ...
        b.usage_count++;
        if (use_rns_multiplication && !b.precomputed) {
            // std::cout << "RNS: modexp" << std::endl;
            #ifdef USE_STRAUS
            rns_modexp::straus(d, {b.c0, b.c1}, {a.y_s, a.y}, crs.n_w_1, crs.rns_ctx);
            #else
            MontInt<N2_LIMBS> d_mont, tmp;
            rns_modexp::sliding_window(tmp, b.c0, a.y_s, crs.n_w_1, crs.rns_ctx);
            rns_modexp::sliding_window(d_mont, b.c1, a.y, crs.n_w_1, crs.rns_ctx);
            crs.rns_ctx->mul_mont(d_mont, tmp, d_mont);
            crs.rns_ctx->from_mont_avx(d, d_mont);
            #endif
        } else {
            if (b.precomputed) {
                // Use precomputed FBE
                ...
                // std::cout << "INT: modexp" << std::endl;
                #ifdef USE_STRAUS
                modexp::straus(d, {b.c0, b.c1}, {a.y_s, a.y}, crs.n_w_1);
                #else
                FMPZ tmp;
