...
def get_records_fuzzy_pake() -> dict[tuple, dict]:
    ...
    for file in glob.glob(f'../evaluate_rns/anike/*_fuzzy_pake_*_stdout.txt'):
        ...
        scheme = file[file.index('../evaluate_rns/anike/') + len('../evaluate_rns/anike/'):file.index('_fuzzy_pake_')]
        ...
        L, W, b, T ,Q = map(int, file.split('_')[-6:-1])
        assert (scheme, L, W, b, T ,Q) not in records, f'Error: Duplicate record for acceleration type {scheme}, L {L}, W {W}, b {b}, T {T}, Q {Q}'
        # record['L'] = L
        # record['W'] = W
        # record['b'] = b
        # record['T'] = T
...
for i, (L, W, b, Q, T) in enumerate(parameters):
    ...
    # Speedup column
    table_cells[0][x_offset + 2] = rf"${format_sigfig(records[('flint', L, W, b, Q, T)]['profile']['setup_time'] / records[('rns', L, W, b, Q, T)]['profile']['setup_time'], sig=2)} \times$"
    table_cells[1][x_offset + 2] = rf"${format_sigfig(records[('flint', L, W, b, Q, T)]['profile']['keygenA_time'] / records[('rns', L, W, b, Q, T)]['profile']['keygenA_time'], sig=2)} \times$"
    table_cells[2][x_offset + 2] = rf"${format_sigfig(records[('flint', L, W, b, Q, T)]['profile']['keygenB_time'] / records[('rns', L, W, b, Q, T)]['profile']['keygenB_time'], sig=2)} \times$"
    table_cells[3][x_offset + 2] = rf"${format_sigfig(records[('flint', L, W, b, Q, T)]['profile']['keyderA_time'] / records[('rns', L, W, b, Q, T)]['profile']['keyderA_time'], sig=2)} \times$"
    table_cells[4][x_offset + 2] = rf"${format_sigfig(records[('flint', L, W, b, Q, T)]['profile']['keyderB_time'] / records[('rns', L, W, b, Q, T)]['profile']['keyderB_time'], sig=2)} \times$"
