...
lines = []
for data_name in mkhss_data_names:
    our_size = getattr(our_scheme, f"{data_name}_size")()
    baseline_size = getattr(baseline_scheme, f"{data_name}_size")()
    saving = baseline_size / our_size
    line = mkhss_line_template.format(
        name='\\DataName' + ''.join(s.capitalize() for s in data_name.split('_')),
        ours=f'{our_size}',
        baseline=f'{baseline_size}' + (' $\\dagger$' if data_name == 'private_share' else ''),
        saving=f'{saving:.1f}'
    )
    lines.append(line)
...
mkhss_table_template = mkhss_table_template.replace('[LINES]', lines)
print(mkhss_table_template)
