...
namespace mkhss {
    constexpr bool IS_BASELINE = false; // Set to true if using the baseline MKHSS scheme
    ...
    struct parameters {
        ...
        int nim_product_bits; // The width of t_A * t_B, plus tau
    ...
    inline void print_parameters(const parameters &params, std::ostream &out = std::cout) {
        out << "# ===== Optimized MKHSS Parameters =====" << std::endl;
        ...
        out << "# nim_product_bits: " << params.nim_product_bits << std::endl;
    ...
    inline void create_parameters(parameters &params,
        ...
        // TODO: with SEI reduction in https://eprint.iacr.org/2024/1950.pdf sqrt(N) reduction, can get w = ~2 when short_exponent_assumption is not used
        ...
        params.bprime_bits = log2_b + stat_sec_param; // log_2(B'), B' = B * 2^{tau}
        // Secret: s_sigma = t_sigma * B' + 1, joint secret: s = s_A * s_B
        params.exponent_bits = 2 * (params.bprime_bits + params.secret_bits) + log2_b + stat_sec_param;
        params.nim_product_bits = 2 * params.secret_bits + stat_sec_param; // The width of t_A * t_B, plus tau
        ...
        if (params.exponent_bits >= (params.n_width * params.w - params.stat_sec_param)) {
            // TODO: there is probably a better way to do this, like mod reduce N^w / 2^i
            params.exponent_bits = NO_MOD_REDUCE; // No mod reduction
        }

        assert(params.nim_product_bits <= (params.n_width * params.w)); // exponent <= plaintext modulus
        if (params.nim_product_bits >= (params.n_width * params.w - params.stat_sec_param)) {
            params.nim_product_bits = NO_MOD_REDUCE; // No mod reduction
        }
    }

    inline void reduce_exponent(FMPZ &dst, const FMPZ &src, int exponent_bits) {
        if (exponent_bits != NO_MOD_REDUCE) {
            fmpz_fdiv_r_2exp(dst.get_fmpz(), src.get_fmpz(), exponent_bits);
        }
    }
    ...
    struct crs {
        ...
        // FMPZModCtx n_w_ctx; // Context for n^w
    ...
    struct public_key {
        ...
        FixedBaseExp g_fbe; // Precomputed FBE for g
        FixedBaseExp f_fbe; // Precomputed FBE for f
    ...
    inline void precompute_share(const crs &crs, public_key &pk, int num_shares) {
        ...
        size_t window_size = fixed_base_exp::get_optimal_window_size(crs.params.sec_param * 2, num_shares);
        if (window_size == 0) {
            // If window size is 0, it means we should not precompute
            return;
        }
        pk.g_fbe.precompute(crs.g, crs.n_w_1, crs.params.sec_param * 2, window_size);
        pk.f_fbe.precompute(pk.f, crs.n_w_1, crs.params.sec_param * 2, window_size);
        pk.precomputed = true;
    }
    ...
    inline size_t get_public_share_size(const crs &crs) {
        return 2 * crs.n_w_1.get_size_in_bytes();
    }
    ...
    inline size_t get_public_share_size(const crs &crs, const public_share &x) {
        ...
        // Serialization must have well-marked boundaries between fields, so need to use values from CRS
        size_t serialized_size = 2 * crs.n_w_1.get_size_in_bytes();
        assert(empirical_size <= serialized_size);
        // std::cerr << "DEBUG: empirical size: " << empirical_size << ", serialized size: " << serialized_size << std::endl;
        return serialized_size;
    }
    ...
    struct input_share {
        ...
        FMPZ c1;
        // Stats
        mutable int usage_count = 0; // How many times this input share was used in an RMS multiplication
        // Optimization to speed up exponentiation
        FixedBaseExp c0_fbe;
        FixedBaseExp c1_fbe;
        bool precomputed = false; // Whether the FBE is precomputed
        // std::optional<std::pair<FixedBaseExp, FixedBaseExp>> fbe = std::nullopt; // c0_fbe, c1_fbe

        // inline input_share() = default;

        // inline input_share(const input_share &other) {
        //     std::cerr << "WARNING: Copying input_share. FBE will not be copied." << std::endl;
        //     c0.set(other.c0);
        //     c1.set(other.c1);
        //     usage_count = other.usage_count;
        //     fbe = std::nullopt; // Copy constructor does not copy FBE
        // }

        // inline void precompute(const crs &crs, int expected_usage_count) {
        //     assert(fbe.has_value() == false && "FBE already precomputed");
        //     size_t window_size = fixed_base_exp::get_optimal_window_size(expected_usage_count);
        //     auto &[c0fbe, c1fbe] = fbe.emplace();
        //     c0fbe.precompute(c0, crs.n_w_1, crs.params.exponent_bits, window_size);
        //     c1fbe.precompute(c1, crs.n_w_1, crs.params.bprime_bits, window_size);
        // }
        ...
        inline void precompute(const crs &crs, int expected_usage_count, int leeway = 0) {
            ...
            c0_fbe.precompute(c0, crs.n_w_1, crs.params.exponent_bits + leeway, fixed_base_exp::get_optimal_window_size(crs.params.exponent_bits, expected_usage_count));
            c1_fbe.precompute(c1, crs.n_w_1, crs.params.bprime_bits + leeway, fixed_base_exp::get_optimal_window_size(crs.params.bprime_bits, expected_usage_count));
            precomputed = true;
        }
    ...
    public:
        ...
        inline void precompute(const crs &crs, int explinencs_count) {
            ...
            size_t window_size = fixed_base_exp::get_optimal_window_size(crs.params.sec_param * 2, explinencs_count);
            if (window_size == 0) {
                // If window size is 0, it means we should not precompute
                return;
            }
            f_fbe.precompute(f, crs.n_w_1, crs.params.sec_param * 2, window_size);
            precomputed = true;
        }
        ...
        inline void mult(const crs &crs, uint64_t inst_id, const input_share &b, const memory_share &a, memory_share &result, std::optional<int> max_log_y = std::nullopt) {
            rms_mult(crs, prf_rms, inst_id, b, a, result, max_log_y);
        }
        ...
        inline void mult_serial(const crs &crs, const input_share &b, const memory_share &a, memory_share &result, std::optional<int> max_log_y = std::nullopt) {
            mult(crs, next_inst_id++, b, a, result, max_log_y);
        }
