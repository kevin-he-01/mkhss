...
namespace baseline_mkhss {
    ...
    struct input_share {
        ...
        inline void precompute(const crs &crs, int expected_usage_count, int leeway = 0) {
            ...
            c0_fbe_n_w_1.precompute(c0, crs.n_w_1, size_n_w_bits + leeway, fixed_base_exp::get_optimal_window_size(size_n_bits * crs.params.w, expected_usage_count)); // <<y*s>> has size log_2(N^w) bits
            c0_prime_fbe_n_2.precompute(c0, crs.n_2, size_n_w_bits + leeway, fixed_base_exp::get_optimal_window_size(size_n_bits * crs.params.w, expected_usage_count)); // <<y*s>> has size log_2(N^w) bits
            c1_fbe_n_w_1.precompute(c1, crs.n_w_1, size_n_bits + leeway, fixed_base_exp::get_optimal_window_size(size_n_bits, expected_usage_count)); // <<y>> has size log_2(N) bits
            c1_prime_fbe_n_2.precompute(c1, crs.n_2, size_n_bits + leeway, fixed_base_exp::get_optimal_window_size(size_n_bits, expected_usage_count)); // <<y>> has size log_2(N) bits
            precomputed = true;
        }
