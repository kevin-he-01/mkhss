...
# mkhss_table_template = r'''
# \begin{tabular}{llll}
# \toprule
#     Data           & Ours (B) & Baseline (B) & Our saving \\
#     \hline
# [LINES]
# \bottomrule
# \end{tabular}
# '''[:-1]
# mkhss_line_template = r'''
#     {name} & {ours} & {baseline} & ${saving} \times$ \\
# '''[:-1]
mkhss_data_names = ['crs', 'private_key', 'public_key', 'private_share', 'public_share', 'synced_input_share', 'memory_share']
barchart_x_coords = ['crs', 'sk', 'pk', 'ss', 'sr', 'sync', 'ms']
assert len(mkhss_data_names) == len(barchart_x_coords), "Data names and x-coordinates must match in length"

# lines = []
for w, n_width, log_b in [(1, N_WIDTH, 1), (1, N_WIDTH, 512), (1, 3968, 1024), (2, 3520, 2048)]:
    our_scheme = MKHSS(
        stat_sec_param=STAT_SEC_PARAM,
        sec_param=SEC_PARAM,
        n_width_bits=n_width,
    ...
    print(f'% log_b = {log_b}')
    for bar_x, data_name in zip(barchart_x_coords, mkhss_data_names):
        our_size = getattr(our_scheme, f"{data_name}_size")()
        print(f'({bar_x}, {our_size / 1000}) ', end='')
    print()  # Newline after each log_b

for w, n_width, log_b in [(3, N_WIDTH, 1)]:
    our_scheme = BaselineMKHSS(
        stat_sec_param=STAT_SEC_PARAM,
        sec_param=SEC_PARAM,
        n_width_bits=n_width,
        log2_b=log_b,
    ...
    print(f'% baseline')
    for bar_x, data_name in zip(barchart_x_coords, mkhss_data_names):
        size = getattr(our_scheme, f"{data_name}_size")()
        print(f'({bar_x}, {size / 1000}) ', end='')
    print()  # Newline after each log_b
sys.exit(0)

# schemes: list[HSS] = [our_scheme, baseline_scheme]
# for scheme in schemes:
#     print('=' * 40)
#     print(f"Scheme: {scheme.__class__.__name__}")
...
#     print(f"w: {scheme.w}")
#     print(f"Log2(B): {scheme.log2_b} bits")
#     if isinstance(scheme, MKHSS):
#         print(f"<<y>> size: {scheme.sub_y_size} bits")
#         print(f"<<y * s>> size: {scheme.sub_ys_size} bits")
#     print('-' * 40)
#     print(f"CRS size: {scheme.crs_size()} bytes")
#     print(f"Private key size: {scheme.private_key_size()} bytes")
#     print(f"Public key size: {scheme.public_key_size()} bytes")
#     print(f"Private share size (min): {scheme.private_share_size()} bytes")
#     print(f"Private share size (max): {scheme.private_share_size(scheme.log2_b)} bytes")
#     print(f"Public share size: {scheme.public_share_size()} bytes")
#     print(f"Synchronized input share size: {scheme.synced_input_share_size()} bytes")
#     print(f"Memory share size: {scheme.memory_share_size()} bytes")

# Location-based ANIKE (ANIKE) using MKHSS
class GeoKE:
    stat_sec_param: int
    sec_param: int
    n_width_bits: int
    ...
    def __init__(self, stat_sec_param: int, sec_param: int, n_width_bits: int, d: int, l: int, hss: Literal['mkhss', 'baseline_mkhss']):
        self.stat_sec_param = stat_sec_param
        self.sec_param = sec_param
        self.n_width_bits = n_width_bits
        self.d = d
    ...
    def anike_pe_alice_size(self) -> int:
        ...
        """
        return self.hss.public_key_size() + (self.l * self.d) * self.hss.public_share_size() + NIKE_PUBLIC_KEY_SIZE_BYTES
    
    def anike_pe_bob_size(self) -> int:
        """
        Calculate the size of the ANIKE public encoding, in bytes
        """
        return self.hss.public_key_size() + (self.l * self.d * 2) * self.hss.public_share_size() + NIKE_PUBLIC_KEY_SIZE_BYTES
    ...
    def anike_st_alice_size(self) -> int:
        ...
        return self.hss.private_key_size() \
                + self.l * self.d * self.hss.private_share_size(x_size_bits=1) + NIKE_PRIVATE_KEY_SIZE_BYTES
    ...
    def anike_st_bob_size(self) -> int:
        ...
        """
        return self.hss.private_key_size() \
                + self.l * self.d * 2 * self.hss.private_share_size(x_size_bits=1) + NIKE_PRIVATE_KEY_SIZE_BYTES
...
for (L, W, b, Q, T) in [(8, 9, 5, 2, 2), (10, 8, 16, 1, 1), (12, 10, 8, 3, 3)]:
    our_fpake = FPAKE(
        stat_sec_param=STAT_SEC_PARAM,
        sec_param=SEC_PARAM,
        n_width_bits=N_WIDTH,
    ...
    print(f'% FPAKE with L={L}, W={W}, b={b}, Q={Q}, T={T}')
    for bar_x, data_name in zip(barchart_x_coords, mkhss_data_names):
        our_size = getattr(our_fpake, f"anike_{data_name}_size")()
        print(f'({bar_x}, {our_size / 1000}) ', end='')
    print()  # Newline after each log_b
